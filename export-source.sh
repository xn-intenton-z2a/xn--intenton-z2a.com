...
# Usage: ./export-source.sh
mkdir -p './exports/'
find "." -type f -not -path '*/programs/*' -not -path '*/results/*' -not -path '*/actions/*' -not -path '*/exports/*' -not -path '*/coverage/*' -not -path '*/node_modules/*' -not -path '*/\.git/*' -not -path '*/\.idea/*' -print | grep -v '.DS_Store' > "./exports/$(date +%Y-%m-%d)-files-list.txt"
...
find "./src" -type f -name '*.html' -print -exec echo "==== Content of {} ====" \; -exec cat {} \; > "./exports/$(date +%Y-%m-%d)-src-cat.txt"
